	...
	switch(a->kind) {
		...
		case (A_opExp):
		{
			...
			if (oper == A_plusOp || oper == A_minusOp || oper == A_timesOp || oper == A_divideOp) {
				...
				//return expTy(Tr_OpArithm(oper, left.exp, right.exp), Ty_Int(notloop));
			}
				...
				// diff from
				//Tr_exp tr_exp = Tr_OpCmp(oper, left.exp, right.exp, left.ty->kind == Ty_string);
		...
		case (A_seqExp):
		{
			...
			Tr_exp res_exp = Tr_Nop();
			struct expty tmp;
			while(cur_exp) {
				tmp = transExp(venv, tenv, cur_exp->head, clevel, breakflag);
				// set to the last exp
				res_ty = tmp.ty;
				res_exp = Tr_Seq(res_exp, tmp.exp);
				cur_exp = cur_exp->tail;
			}
			return expTy(res_exp, res_ty);
		}
		case (A_assignExp):
		{
		...
		case (A_letExp):
		{
			...
			for (d = a->u.let.decs; d; d = d->tail) {
				transDec(venv, tenv, d->head, clevel, breakflag);
			}
	...
	switch (d->kind) {
		case (A_functionDec):
		{
			...
			for (funcL=d->u.function; funcL; funcL=funcL->tail) {
				...
				//Tr_Func(res.exp, funcEntry->u.fun.level);
			}
